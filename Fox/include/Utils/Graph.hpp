...
#ifndef FOXENGINE_GRAPH_HPP
#define FOXENGINE_GRAPH_HPP
...
// a very simple directional graph
    template<typename NodeType, typename EdgeType = Edge>
    class Graph
    {
    public:
        ...
        Span<const EdgeType> edges() const;
        IdMap<EdgeType>& edges();
    ...
    private:
        ...
        IdMap<EdgeType> edges_;
    };

    template<typename NodeType, typename EdgeType>
    NodeType& Graph<NodeType, EdgeType>::node(const int id)
    {
        return const_cast<NodeType&>(static_cast<const Graph*>(this)->node(id));
    }

    template<typename NodeType, typename EdgeType>
    const NodeType& Graph<NodeType, EdgeType>::node(const int id) const
    {
    ...
    template<typename NodeType, typename EdgeType>
    Span<const int> Graph<NodeType, EdgeType>::neighbors(int node_id) const
    {
    ...
    template<typename NodeType, typename EdgeType>
    Span<const EdgeType> Graph<NodeType, EdgeType>::edges() const
    {
        return edges_.elements();
    }

    template<typename NodeType, typename EdgeType>
    IdMap<EdgeType>& Graph<NodeType, EdgeType>::edges()
    {
        return edges_;
    }

    template<typename NodeType, typename EdgeType>
    size_t Graph<NodeType, EdgeType>::num_edges_from_node(const int id) const
    {
    ...
    template<typename NodeType, typename EdgeType>
    int Graph<NodeType, EdgeType>::insert_node(const NodeType& node)
    {
    ...
    template<typename NodeType, typename EdgeType>
    void Graph<NodeType, EdgeType>::erase_node(const int id)
    {
        ...
        // first, remove any potential dangling edges
        {
            ...
            for (const EdgeType& edge : edges_.elements())
            {
                if (edge.contains(id))
                {
    ...
    template<typename NodeType, typename EdgeType>
    int Graph<NodeType, EdgeType>::insert_edge(const int from, const int to)
    {
        const int id = current_id_++;
        assert(!edges_.contains(id));
        assert(nodes_.contains(from));
        assert(nodes_.contains(to));
        edges_.insert(id, EdgeType(id, from, to));
    ...
    template<typename NodeType, typename EdgeType, typename Visitor>
    void dfs_traverse(const Graph<NodeType, EdgeType>& graph, const int start_node, Visitor visitor)
    {
        std::stack<int> stack;
