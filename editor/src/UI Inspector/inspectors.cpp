...
bool inspect_ui_var(rttr::variant& var, bool skip_custom, bool read_only,
                 const inspector::meta_getter& get_metadata)
{
    ...
    {
        changed |= inspector->inspect_imgui(var, read_only, get_metadata);
    }
    ...
    else
    {
        ...
        {
            // If the property have NO_UI metadata, so don't draw it
            if (prop.get_metadata("NO_UI"))
                return false;
            ...
            if(open)
            {
                ...
                {
                    prop_changed |= inspect_ui_var(prop_var, false, is_readonly, get_meta);
                }
...
bool inspect_track_var(rttr::variant& var, bool skip_custom, bool read_only,
                    const inspector::meta_getter& get_metadata)
{
    rttr::instance object = var;
    auto type = object.get_derived_type();
    ...
    if(!skip_custom && inspector)
    {
        changed |= inspector->inspect_track(var, read_only, get_metadata);
    }
    else if(properties.empty())
    {
        if(type.is_enumeration())
...
//                else
//                {
                    prop_changed |= inspect_track_var(prop_var, false, is_readonly, get_meta);
//                }
...
bool inspect_array(rttr::variant& var, bool read_only, const inspector::meta_getter& get_metadata)
{
    ...
    for(std::size_t i = 0; i < size; ++i)
    {
        ...
        changed |= inspect_ui_var(value, false, read_only, get_metadata);
...
bool inspect_enum(rttr::variant& var, rttr::enumeration& data, bool read_only)
{
    ...
    else
    {
        ...
        if(ImGui::Combo("", &listbox_item_current, cstrings.data(), listbox_item_size, listbox_item_size))
        {
