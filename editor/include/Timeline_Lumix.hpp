//
// Created by samuel on 16/07/2021.
//

#ifndef FOXENGINE_TIMELINE_LUMIX_HPP
#define FOXENGINE_TIMELINE_LUMIX_HPP
...
namespace ImGui
{
    ...
    inline bool TimelineEvent(const char *str_id, double &val1, double &val2)
    {
        ...
        ImVec2 cursor_pos(GetWindowContentRegionMin().x + win->Pos.x + columnOffset - TIMELINE_RADIUS,
                          win->DC.CursorPos.y);
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            pos.x +=
                    s_time_scale * columnWidth * float(values[i]) / s_max_timeline_value - columnWidth * s_time_offset +
                    TIMELINE_RADIUS;
            ...
            if (active || IsItemHovered()) {
                ...
                float endy = parent_win->ContentRegionRect.Max.y +
                             win->Pos.y; // draw all the way to the bottom of the parent window
    ...
    inline void EndTimeline(int num_vertical_grid_lines, double &time_in, double &time_out)
    {
        ...
        win->DrawList->AddRectFilled(start, end,
                                     IsItemActive() || IsItemHovered() ? pz_active_color : pz_inactive_color);
...
#endif //FOXENGINE_TIMELINE_LUMIX_HPP
